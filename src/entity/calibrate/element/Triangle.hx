...
class Triangle extends luxe.Sprite {
	...
	public var radius_des: Float = 64;

	override public function new() {
		super({
			name: 'element.triangle',
			name_unique: true,
			pos: new Vector(Main.w * 2/4, Main.h * 1/4),
			visible: false,
			color: Main.c3.clone( ),
		});
		...
		this.add(new component.Hitbox({
			name: 'hitbox',
			body: luxe.collision.shapes.Polygon.create(pos.x, pos.y, 3, this.radius_des)
		}));
	...
	public function activate_move() {
		...
		Actuate.tween(this.pos, growTime, {x: Main.w/2, y: Main.h * 2/3}).delay(delayTime).onComplete(function(){
			this.destroy();
			Luxe.events.fire('add.element.triangle');
		});
	}
